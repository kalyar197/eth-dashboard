...
"""
Ethereum dominance calculator using REAL market cap data
...
def get_metadata():
    ...
    return {
        ...
        'strokeWidth': 2,
        'description': 'Ethereum market cap dominance - REAL DATA from CoinGecko'
    }
...
def get_data(days='365'):
    """Fetches ETH dominance using REAL market cap data from CoinGecko"""
    ...
    try:
        ...
        if 'ethereum' not in market_caps:
            ...
            return {
                'metadata': metadata,
                'data': [],
                'error': 'Could not fetch ETH market cap from CoinGecko'
            }
        
        # Calculate dominance
        eth_mc = market_caps['ethereum']
        other_coins = [market_caps[coin] for coin in market_caps.keys() if coin != 'ethereum']
        dominance_data = calculate_dominance_from_market_caps(eth_mc, other_coins)
