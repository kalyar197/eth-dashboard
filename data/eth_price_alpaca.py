# data/eth_price_alpaca.py
"""
Ethereum (ETH/USD) price data fetcher using Alpaca API
...
# Note: Using environment variables or config file for API keys
try:
    from config import ALPACA_API_KEY, ALPACA_SECRET_KEY
    ALPACA_CONFIGURED = True
except ImportError:
    print("[ETH Price Alpaca] Warning: Alpaca API keys not configured in config.py")
    ALPACA_CONFIGURED = False

def get_metadata():
    """Returns metadata describing how this data should be displayed"""
    return {
        'label': 'ETH Price (vs BTC)',
        'yAxisId': 'indicator',
        ...
        'strokeWidth': 2,
        'description': 'ETH/USD price oscillator',
        'data_structure': 'simple'
    }

def fetch_from_alpaca(start_date, end_date):
    """
    Fetch ETH/USD OHLCV data from Alpaca API for a specific date range.
    ...
    print(f"[ETH Price Alpaca] Fetching ETH/USD from Alpaca: {start_date.date()} to {end_date.date()}")
    ...
    try:
        ...
        print(f"[ETH Price Alpaca] Successfully fetched {len(raw_data)} data points")
        if raw_data:
            print(f"[ETH Price Alpaca] Sample: timestamp={raw_data[0][0]}, close=${raw_data[0][1]:.2f}")

        return raw_data

    except Exception as e:
        print(f"[ETH Price Alpaca] Error fetching from Alpaca: {e}")
        raise

def get_data(days='1095', asset='btc'):
    """
    Fetches ETH/USD price data using incremental fetching strategy.
    Returns simple format [timestamp, close_price] for oscillator calculations.
    ...
    try:
        ...
        if historical_data:
            ...
            print(f"[ETH Price Alpaca] Incremental fetch from {start_date.date()} to {end_date.date()}")
            ...
            print(f"[ETH Price Alpaca] Full fetch from {start_date.date()} to {end_date.date()}")
            new_data = fetch_from_alpaca(start_date, end_date)
        ...
        print(f"[ETH Price Alpaca] Returning {len(filtered_data)} records")
        if filtered_data:
            ...
            print(f"[ETH Price Alpaca] Date range: {start_dt} to {end_dt}")
        ...
        print(f"[ETH Price Alpaca] Error in get_data: {e}")

        # Fallback to historical data
        historical_data = load_historical_data(dataset_name)
        if historical_data:
            print(f"[ETH Price Alpaca] Falling back to historical data ({len(historical_data)} records)")
        ...
        # No data available
        print(f"[ETH Price Alpaca] No historical data available for fallback")
        return {
            'metadata': metadata,
            'data': []
        }
